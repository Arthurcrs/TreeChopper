...
public class CommonProxy {
	...
	public void InteractWithTree(PlayerInteractEvent interactEvent) {
		...
		if (interactEvent.getSide().isClient()
				&& m_PlayerPrintNames.containsKey(interactEvent.getEntityPlayer().getPersistentID())
				&& m_PlayerPrintNames.get(interactEvent.getEntityPlayer().getPersistentID())) {
			interactEvent.getEntityPlayer().sendMessage(new TextComponentTranslation(I18n.format("proxy.printBlock")
					+ " "
					+ interactEvent.getWorld().getBlockState(interactEvent.getPos()).getBlock().getUnlocalizedName()));
			interactEvent.getEntityPlayer().sendMessage(new TextComponentTranslation(I18n.format("proxy.printMainHand")
					+ " " + interactEvent.getEntityPlayer().getHeldItemMainhand().getUnlocalizedName()));
		}
		...
		if (CheckWoodenBlock(interactEvent.getWorld(), interactEvent.getPos())
				&& CheckItemInHand(interactEvent.getEntityPlayer()) && shifting) {

			int axeDurability = interactEvent.getEntityPlayer().getHeldItemMainhand().getMaxDamage()
					- interactEvent.getEntityPlayer().getHeldItemMainhand().getItemDamage();

			if (m_PlayerData.containsKey(interactEvent.getEntityPlayer().getPersistentID())
					&& m_PlayerData.get(interactEvent.getEntityPlayer().getPersistentID()).m_BlockPos
							.equals(interactEvent.getPos())
					&& m_PlayerData
			...
			logCount = treeHandler.AnalyzeTree(interactEvent.getWorld(), interactEvent.getPos(),
					interactEvent.getEntityPlayer());

			/*
			 * System.out.println("Max damage: " +
			 * interactEvent.getEntityPlayer().getHeldItemMainhand().getMaxDamage());
			 * System.out.println("Item damage: " +
			 * interactEvent.getEntityPlayer().getHeldItemMainhand().getItemDamage());
			 */

			if (interactEvent.getEntityPlayer().getHeldItemMainhand().isItemStackDamageable()
					&& axeDurability < logCount) {
				m_PlayerData.remove(interactEvent.getEntityPlayer().getPersistentID());
				return;
			}

			if (logCount > 1) {
				m_PlayerData.put(interactEvent.getEntityPlayer().getPersistentID(),
						new PlayerInteract(interactEvent.getPos(), logCount, axeDurability));
			}
		} else {
			m_PlayerData.remove(interactEvent.getEntityPlayer().getPersistentID());
		}
		...
		if (m_PlayerData.containsKey(breakSpeed.getEntityPlayer().getPersistentID())) {
			...
			if (blockPos.equals(breakSpeed.getPos())) {
				breakSpeed.setNewSpeed(breakSpeed.getOriginalSpeed()
						/ (m_PlayerData.get(breakSpeed.getEntityPlayer().getPersistentID()).m_LogCount / 2.0f));
			} else {
				breakSpeed.setNewSpeed(breakSpeed.getOriginalSpeed());
			}
		}
		...
		if (m_PlayerData.containsKey(breakEvent.getPlayer().getPersistentID())) {
			...
			if (blockPos.equals(breakEvent.getPos())) {
				...
				if (!breakEvent.getPlayer().isCreative()
						&& breakEvent.getPlayer().getHeldItemMainhand().isItemStackDamageable()
						&& ConfigurationHandler.durabilityLossFactor != 0) {

					int axeDurability = breakEvent.getPlayer().getHeldItemMainhand().getItemDamage()
							+ (int) (m_PlayerData.get(breakEvent.getPlayer().getPersistentID()).m_LogCount * 1.5
									* ConfigurationHandler.durabilityLossFactor);
	...
	protected boolean CheckWoodenBlock(World world, BlockPos blockPos) {
		...
		if (ConfigurationHandler.blockWhiteList
